...
function Dropdown<T extends string>({ label, value, options, onChange, required, error, placeholder, disabled, disabledMessage }: DropdownProps<T>) {
  ...
  const handleClick = () => {
    if (disabled) {
      ...
      setTimeout(() => setShowDisabledHint(false), 2000);
      return;
    }
    setIsOpen(!isOpen);
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {showDisabledHint && disabledMessage && (
        <div className="absolute top-full left-0 mt-2 px-3 py-2 min-w-[200px]
          bg-amber-500/20 border border-amber-500/30 rounded-lg shadow-lg z-50
...
export default function TransactionInput({ onSubmit }: TransactionInputProps) {
  ...
  // Merchant options - filtered by selected category
              ...
              <div className="flex flex-wrap items-center gap-2 p-3 -mx-1 bg-primary/5 rounded-xl border border-primary/10">
                <Dropdown
                  ...
                  onChange={(val) => {
                    ...
                    setShowCategoryError(false);
                    // Clear merchant when category changes (merchant may not exist in new category)
                ...
                <Dropdown
                  ...
                  disabledMessage={t('selectCategoryFirst')}
                />
              </div>
